...
def run(API, environ, indata, session):
    ...
    while ny > 1970:
        ...
        query = {
            "query": {
                "bool": {
                    "must": [
                        {"range": {"ts": {"from": tf, "to": t}}},
                        {"term": {"organisation": dOrg}},
                    ],
                    "must_not": [{"match": {"sourceURL": "commits*"}}],
                }
            }
        }
        ...
        query["aggs"] = {"by_sender": {"terms": {"field": "sender", "size": 2500}}}
    ...
    JSON_OUT = {
        "text": "This shows Pony Factors as calculated over a %u month timespan. Authorship is a measure of the people it takes to make up the bulk of email traffic, and meta-pony is an estimation of how many organisations/companies are involved."
        % hl,
        "timeseries": ts,
        "okay": True,
