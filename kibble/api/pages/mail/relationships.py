...
def run(API, environ, indata, session):
    ...
    query["aggs"] = {"per_ml": {"terms": {"field": "sourceID", "size": 10000}}}
    ...
    for doc in res["aggregations"]["per_ml"]["buckets"]:
        ...
        query["aggs"] = {"per_ml": {"terms": {"field": "sender", "size": 10000}}}
        ...
        for xID, xrepo in repos.items():
            if xID in repodatas:
                ...
                if indata.get("collapse"):
                    ...
                    xID = re.sub(
                        r"^.+/(?:list\.html\?)?", "", xrepodata["_source"]["sourceURL"]
                    )
                if xID != ID:
                    xlinks = []
                    for author in xrepo:
    ...
    for source_id in repo_notoriety:
        ...
        doc = {
            "id": source_id,
            "name": source_id,
            "emails": repo_commits[source_id],
            "authors": asize,
            "links": lsize,
            "size": max(5, (1 - abs(math.log10(asize / max_authors))) * 45),
            "tooltip": "%u connections, %u contributors, %u emails"
