...
def run(API, environ, indata, session):
                ...
                "must": [
                    {
                        "range": {
                            "date": {
                                "from": time.strftime(
                                    "%Y/%m/%d %H:%M:%S", time.localtime(dateFrom)
                                ),
                                "to": time.strftime(
                                    "%Y/%m/%d %H:%M:%S", time.localtime(dateTo)
                                ),
                            }
                        }
                    },
    ...
    query["aggs"] = {"authors": {"cardinality": {"field": "sender"}}}
    res = session.DB.ES.search(
                ...
                "must": [
                    {
                        "range": {
                            "date": {
                                "from": time.strftime(
                                    "%Y/%m/%d %H:%M:%S", time.localtime(dateYonder)
                                ),
                                "to": time.strftime(
                                    "%Y/%m/%d %H:%M:%S", time.localtime(dateFrom - 1)
                                ),
                            }
                        }
                    },
