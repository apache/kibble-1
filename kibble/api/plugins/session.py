...
class KibbleSession:
    ...
    def logout(self):
        ...
        if self.user and self.cookie:
            ...
            self.headers.append(
                ("Set-Cookie", cookies["kibble_session"].OutputString())
            )
            try:
                self.DB.ES.delete(
                    index=self.DB.dbname, doc_type="uisession", id=self.cookie
                )
                self.cookie = None
                self.user = None
    ...
    def __init__(self, DB, environ, config):
        ...
        if "HTTP_KIBBLE_TOKEN" in environ:
            ...
            if re.match(
                r"^[-a-f0-9]+$", token
            ):  # Validate token, must follow UUID4 specs
                res = self.DB.ES.search(
                    index=self.DB.dbname,
                    doc_type="useraccount",
                    body={"query": {"match": {"token": token}}},
                )
                if res["hits"]["hits"]:
            ...
            if cookies and "kibble_session" in cookies:
                ...
                try:
                    if re.match(
                        r"^[-a-f0-9]+$", cookie
                    ):  # Validate cookie, must follow UUID4 specs
                        doc = None
                        sdoc = self.DB.ES.get(
                            index=self.DB.dbname, doc_type="uisession", id=cookie
                        )
                        ...
                        if doc and "_source" in doc:
                            ...
                            if age > (7 * 86400):
                                ...
                                sdoc["_source"]["timestamp"] = int(
                                    time.time()
                                )  # Update timestamp in session DB
                                self.DB.ES.update(
                                    index=self.DB.dbname,
                                    doc_type="uisession",
                                    id=cookie,
                                    body={"doc": sdoc["_source"]},
                                )
                            if doc:
