...
var API, APIVERSION, Chart, HTML, Row, Widget, activateview, addorguser, addsources, affiliate, affiliation, affiliationWizard, altemail, app, badModal, bio, chartOnclick, chartToSvg, chartWrapperButtons, charts_donutchart, charts_linechart, charts_linechart_stacked, charts_radarchart, cog, comstat, copyCSS, currentSources, dataTable, datepicker, datepickers, deletesource, doResetPass, donut, downloadBlob, explorer, fScreen, fetch, fetchPhonebook, filterPerson, filterView, findWidget, genColors, get, getResetToken, globArgs, hsl2rgb, imexplorer, isArray, isHash, issueexplorer, kibbleLogin, kibbleLoginCallback, linechart, loadPageWidgets, logexplorer, login, mailexplorer, manageviews, messages, mk, multiviewexplorer, mvp, newview, orgadmin, pageID, paragraph, phonebook_cached, post, postPublishLink, preferences, pubWidget, publishWidget, publisher, publisherPublic, publisherWidget, put, pwReset, quickColors, radar, radarIndicators, rcollate, redirs, relationship, remail, remorguser, renderAccountInfo, renderPhonebook, report, resetpw, rotateTable, saveprefs, saveview, sendEmail, set, setupPage, setupPhonebook, showMore, signup, snoot_sidebar, snoot_sidebar_hide, sourcelist, sourceret, stackChart, swi, switchChartType, tagList, tagsource, theme, toFullscreen, toNormal, top5, treemap, trend, trendBox, txt, updateTimeseriesWidgets, updateWidgets, userAccount, validateLogin, validateSignup, viewJS, viewexplorer, widgetCache, widgetexplorer, worldmap, xxCharts,
...
Chart = (function() {
  function Chart(parent, type, data, options) {
    var btn, btnDiv, btns, chartWrapperColors, cid, el, hObj, i, inner, len, len1, q, ref, ref1, ref2, ref3, u;
    ...
    if (type === 'radar') {
      ref3 = charts_radarchart(this.chartdiv, data), this.chartobj = ref3[0], this.config = ref3[1];
    }
    return this.main;
  }
...
explorer = function(json, state) {
  var chk, ezURL, h, id, item, label, len, len1, list, m, opt, org, q, ref, ref1, ref2, ref3, slen, u;
...
mailexplorer = function(json, state) {
  var ezURL, h, item, len, len1, list, m, opt, org, q, ref, ref1, slen, u;
  org = json.organisation;
  h = document.createElement('h2');
  if (json.tag) {
    org.name += " (Filter: " + json.tag + ")";
...
issueexplorer = function(json, state) {
  var ezURL, h, item, len, len1, list, m, n, opt, org, q, ref, ref1, ref2, ref3, slen, u;
  org = json.organisation;
  if (json.tag) {
    org.name += " (Filter: " + json.tag + ")";
  }
...
multiviewexplorer = function(json, state) {
  var h, item, k, len, list, opt, org, q, ref, results, tName, u;
  org = json.organisation;
  h = document.createElement('h2');
  ...
  for (k = q = 1; q <= 3; k = ++q) {
    ...
    results.push(list.addEventListener("change", function() {
      var source, x;
      source = this.value;
      if (source === "") {
        source = null;
...
viewexplorer = function(json, state) {
  var h, item, len, list, opt, org, q, ref, tName;
  org = json.organisation;
  h = document.createElement('h2');
...
setupPage = function(json, state) {
  ...
  for (q = 0, len = ref.length; q < len; q++) {
    ...
    row = new Row();
    results.push((function() {
      var len1, ref1, ref2, ref3, results1, u;
      ref1 = r.children;
      results1 = [];
      for (u = 0, len1 = ref1.length; u < len1; u++) {
        child = ref1[u];
...
deletesource = function(hash) {
  ...
  if (window.confirm("Are you sure you wish to delete this resource?\nNOTE: Not everything about this can be deleted. Due to the nature of especially git, the unique commit stats will NOT change when you delete a git resource until you force a complete reindex of everything.")) {
    ...
    return postJSON('manage-sources', {
      action: 'delete',
      source: hash
    }, null, null);
  }
...
sourcelist = function(json, state) {
  var a, aa, ab, ac, act, ad, ae, apass, apassdiv, apassinput, apasslabel, atype, atypediv, atypeinput, atypelabel, auser, auserdiv, auserinput, auserlabel, auth, borked, btn, cf, cl, color, d, dbtn, desc, div, div2, el, firstRun, form, fs, h, h1, h2, ic, icons, item, label, label2, lastException, lastFailure, lastUpdate, len, len1, len2, len3, len4, len5, len6, lg, lu, m, opt, org, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, retval, running, slist, slistdiv, slistinput, slistlabel, source, sources, status, steps, stype, stypediv, stypeinput, stypelabel, sw, sw2, t, tbl, tbody, td, thead, tr, tt, type, types, u;
  slist = mk('div');
  ref = json.sources;
  for (org in ref) {
    sources = ref[org];
    ...
    thead = mk('thead');
    tr = mk('tr');
    ref1 = ['Type', 'Source', 'Tag(s)', 'Progress', 'Last Update', 'Status', 'Actions'];
    ...
    app(tbl, thead);
    tbody = mk('tbody');
    for (u = 0, len1 = sources.length; u < len1; u++) {
      source = sources[u];
      source.good = true;
    ...
    for (ab = 0, len3 = sources.length; ab < len3; ab++) {
      source = sources[ab];
      d = mk('tr');
      ...
      if ((ref4 = source.type) === 'jira' || ref4 === 'bugzilla') {
        steps = ['issues'];
      }
      if ((ref5 = source.type) === 'gerrit' || ref5 === 'gitlab' || ref5 === 'github') {
        steps = ['sync', 'census', 'count', 'evolution', 'issues'];
      }
      if ((ref6 = source.type) === 'irc' || ref6 === 'stats') {
        steps = ['census'];
      }
      for (ac = 0, len4 = steps.length; ac < len4; ac++) {
        item = steps[ac];
        color = "#394";
        cl = icons[item];
  ...
  for (ad = 0, len5 = types.length; ad < len5; ad++) {
    type = types[ad];
    opt = mk('option');
    opt.text = type;
    opt.value = type;
  ...
  for (ae = 0, len6 = types.length; ae < len6; ae++) {
    type = types[ae];
    opt = mk('option');
    opt.text = type;
    opt.value = type;
  ...
  sw.addEventListener('click', function() {
    if (this.checked) {
      return auth.style.display = 'block';
    } else {
      return auth.style.display = 'none';
    }
  });
  return state.widget.inject(form);
};
...
relationship = function(json, state) {
  var aa, ab, catdata, categories, cats, colors, dates, div, echartLine, factor, filled, graph, item, k, len, len1, len2, len3, link, list, node, opt, option, p, q, range, rect, ref, ref1, ref2, ref3, stack, tName, type, u;
  div = document.createElement('div');
  cats = new Array();
  dates = new Array();
  catdata = {};
  ...
  factor = Math.min((div.style.minHeight.match(/px/) && parseInt(div.style.minHeight)) || (div.style.height.match(/px/) && parseInt(div.style.height)) || window.innerHeight, (div.style.minWidth.match(/px/) && parseInt(div.style.minWidth)) || (div.style.width.match(/px/) && parseInt(div.style.width)) || window.innerWidth);
  ref = json.nodes;
  for (k = q = 0, len = ref.length; q < len; k = ++q) {
    node = ref[k];
    node.draggable = true;
    ...
    if (location.href.match(/reltype=galaxy/) || (state && state.config && state.config.reltype === 'galaxy')) {
    ...
    if (location.href.match(/reltype=circle/) || (state && state.config && state.config.reltype === 'circle')) {
      node.x = node.y = null;
    }
  }
  if (location.href.match(/reltype=galaxy/) || (state && state.config && state.config.reltype === 'galaxy')) {
  ...
  option = {
    ...
    legend: [
      {
        data: categories.map((function(_this) {
          return function(a) {
    ...
    series: [
      {
        ...
        type: 'graph',
        layout: (location.href.match(/reltype=galaxy/) || (state && state.config && state.config.reltype === 'galaxy') ? 'force' : (location.href.match(/reltype=circle/) || (state && state.config && state.config.reltype === 'circle') ? 'circular' : 'none')),
        data: graph.nodes,
        links: graph.links,
        ...
        lineStyle: {
          normal: {
            color: 'source',
            curveness: location.href.match(/reltype=galaxy/) || (state && state.config && state.config.reltype === 'galaxy') ? 0 : 0.3
          }
        },
        force: location.href.match(/reltype=galaxy/) || (state && state.config && state.config.reltype === 'galaxy') ? {
          repulsion: 75
        } : {}
      }
    ]
  ...
  if (!json.widget) {
    ...
    state.widget.inject(list);
    ref3 = ['standard', 'galaxy', 'circle'];
    for (ab = 0, len3 = ref3.length; ab < len3; ab++) {
      item = ref3[ab];
      opt = document.createElement('option');
      opt.value = item;
      opt.text = item;
      if (globArgs[tName] && globArgs[tName] === item) {
        opt.selected = 'selected';
      }
      list.appendChild(opt);
    }
    list.addEventListener("change", function() {
      ...
      x = {};
      x[tName] = source;
      return updateWidgets('relationship', null, x);
    }, false);
...
top5 = function(json, state) {
  var aa, ab, ac, ad, changes, committer, data, from, i, id, item, len, len1, obj, obj2, p, people, person, plink, q, ref, ref1, ref2, ref3, ref4, ref5, ref6, to, u;
  people = [];
  if (state.widget.args.source === 'mail-subjects') {
    ...
    i = 0;
    ref1 = json.top;
    for (aa = 0, len1 = ref1.length; aa < len1; aa++) {
      item = ref1[aa];
      i++;
      if (i > 5) {
        break;
    ...
    if (state.widget.args.source === 'committers' || (json.widget && json.widget.match(/commit/))) {
      ref2 = json.people;
      for (person in ref2) {
        data = ref2[person];
    ...
    state.widget.inject(obj, true);
    if (people.length > 5 && !json.widget) {
      id = Math.floor(Math.random() * 987654321).toString(16);
      obj.innerHTML += "<p><a href='javascript:void(0);' onclick='this.style.display=\"none\"; showMore(\"" + id + "\");'>Show more</a></p>";
      obj2 = document.createElement('div');
      obj2.setAttribute("id", id);
      obj2.style.display = "none";
      for (i = ad = 5; ad <= 9; i = ++ad) {
        if (people[i]) {
          committer = json.people[people[i][0]];
          if (committer) {
            committer.count = people[i][1].commits;
    ...
    if (people.length === 0) {
      return obj.innerHTML = "No data found for this period.";
    }
  }
};
...
activateview = function(id, url) {
  id = id || "";
  if (!url) {
    url = "?page=views";
  }
  return postJSON('views', {
    activate: id
  }, null, function() {
